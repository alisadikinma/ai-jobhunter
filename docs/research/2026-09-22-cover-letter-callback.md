...
| Sign-off: `Sincerely,` / `Best regards,` / `Kind regards,` | Not in either raw file at all — no source here names a sign-off. These are common business-letter conventions chosen for the template, with no citation behind them. | **None** (convention only — the one format rule with no cited source) |
